...
void Con_DrawSolidConsole( int lines )
{
	...
	if( host.allow_console )
	{
		...
		Q_snprintf( curbuild, MAX_STRING, "Half-Life 1.0 alpha 623", PROTOCOL_VERSION, XASH_VERSION, Q_buildnum( ));
...
void Con_DrawVersion( void )
{
	...
	if( host.force_draw_version || draw_version )
		Q_snprintf( curbuild, MAX_STRING, "Half-Life 1.0 alpha 623", PROTOCOL_VERSION, XASH_VERSION, Q_buildnum( ));
	else Q_snprintf( curbuild, MAX_STRING, "1.0 alpha 623", PROTOCOL_VERSION, XASH_VERSION, Q_buildnum( )); 
	Con_DrawStringLen( curbuild, &stringLen, &charH );
